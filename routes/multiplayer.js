...
const { TEACHER_QUIZZES } = require('../game/teacher-quizzes');
...
// ── Get teacher quizzes (only verified educators) ────────────────────────────
router.get('/teacher-quizzes', (req, res) => {
  const email = req.session?.email || '';
  const role = req.session?.role || 'guest';
  const isVerified = isEducatorEmail(email) || role === 'admin';

  if (!isVerified) {
    return res.status(403).json({
      error: 'Teacher quizzes are only available to verified educators.',
      hint: 'Register with a school email (.edu, .sch, .ac) to access these quizzes.',
    });
  }

  // Return quiz metadata only (not questions) for listing
...
// ── Get single teacher quiz with questions ───────────────────────────────────
...
router.get('/educator-status', (req, res) => {
  const email = req.session?.email || '';
  res.json({
    isVerified: isEducatorEmail(email),
